...
class IdPrimaryKeyMixin(object):
    #An ID column we wish to add to all rows, it will be unique to the table and given incrementaly
...
class Account(Base, IdPrimaryKeyMixin, DateTimeMixin):
    ...
    def __repr__(self):
        ret = "<ID: %d, username: %s, email: %s, token: %s>" % (self.id, self.username, self.email, self.account_token)
        return ret
...
class Donor(Base, IdPrimaryKeyMixin, DateTimeMixin):
    ...
    # Columns:
    related_account = Column(Integer, nullable = False) #So we can easily locate the user information
    donations_given = Column(Integer, default=0) #The number of requests this donor has closed (been the last one to donate to)
    money_given = Column(Float, default=0) #The total amount of money this donor has donated

    def __repr__(self):
        ret = "<ID: %d, donations_given: %d, money_given: %d>" % (self.id, self.donations_given, self.money_given)
        return ret
